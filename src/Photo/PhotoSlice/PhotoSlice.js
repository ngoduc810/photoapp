...
const photo = createSlice({
    ...
    reducers: {
        ...
        removePhoto: (state, action) => {
            return state.filter( state => state.photo !== action.payload)
        },
        editPhotoId: (state, action) => {
            const photoIndex = state.findIndex(e => e.id === action.payload.id)
...
export const { addPhoto,removePhoto,editPhotoId } = actions;
